    ...
    public class OrderServices : IOrderServices
    {
        ...
        public async Task<ResponseDto<object>> AddOrder(CreateOrderDto dto)
        {
            try
            {
                ...
                var table = await _tableRepository.GetByIdAsync(result.TableId);
        ...
        public async Task<ResponseDto<List<ResultOrderDto>>> GetAllOrders()
        {
            try
            {
                ...
                var orderıtemdb = await _orderItemRepository.GetAllAsync();
                if (orderdb.Count == 0)
                {
        ...
        public async Task<ResponseDto<object>> UpdateOrderStatusIptalEdildi(int orderId)
        {
            try
            {
                ...
                return new ResponseDto<object>
                {
                    Success = true,
                    Data = null,
                    Message = "Sipariş Durumu İptal Edildi Olarak Güncellendi."
                };
            }
            catch (Exception)
            {
